...
namespace z_dht::bencode {
    namespace {
        ...
        std::vector<Value> dencode(const std::string &value) {
            if (value.empty()) {
                throw std::runtime_error("Cannot decode an empty Bencode string");
            }
            vector<Value> result;
            auto typeStack = std::vector<Type>();
            auto charStack = std::vector<char>();
            for (int i = 0; i < value.length(); ++i) {
                auto c = value[i];
                if (c == 'i') {
                    typeStack.push_back(Type::Integer);
                }

                if (c == 'l') {
                    typeStack.push_back(Type::List);
                }
                ...
                if (c == 'e') {
                    auto tp = typeStack.back();
                    if (tp == Type::Integer) {
                        std::string str(charStack.begin(), charStack.end());
                        result.emplace_back(stoll(str));
                    } else if (tp == Type::List) {
                        //直接跳过
                    } else if (tp == Type::Dictionary) {
                        //skip
                    }
                }
                //是数字，说明是，就进行一次获取头部
                if (std::isdigit(c)) {
                    ...
                    while (true) {
                        i++;
                        if (value[i] == ':') {
                            std::string str(len.begin(), len.end());
                            auto l = stoll(str);
                            auto s = value.substr(i - l, i);
